...
# 보간 설정
STEP_DURATION = 0.1  # (Normal) Power Stroke 시간
RETURN_STEP_DURATION = STEP_DURATION * 5 # (Normal) Return Stroke 시간
...
class Poses:
    ...
    # --- 좌회전에 사용되는 위치 ---
    TURN_LEFT_POS_1 = [2861, 2141, 2228]  # = pos3
    ...
    TURN_RIGHT_POS_5 = [1248, 2141, 2228]  # = pos5
...
class DynamixelController:
    ...
    def _write_1byte_txrx(self, motor_id, address, value):
        ...
        if dxl_comm_result != COMM_SUCCESS:
            print(f"%s" % self.packet_handler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print(f"%s" % self.packet_handler.getRxPacketError(dxl_error))
    ...
    def sync_write_goal_position(self, motor_positions: dict):
        ...
        for motor_id, position in motor_positions.items():
            ...
            add_param_result = self.group_sync_write.addParam(motor_id, param_goal_position)
        ...
        if dxl_comm_result != COMM_SUCCESS:
            print(f"sync_write_goal_position txPacket failed: {self.packet_handler.getTxRxResult(dxl_comm_result)}")
            return False
        return True
...
class QuadrupedRobot:
    ...
    def __init__(self, controller: DynamixelController, leg_ids: dict):
        ...
        self.current_positions = {leg_num: list(Poses.INITIAL) for leg_num in self.leg_ids.keys()}
    ...
    def _move_legs_interpolated(self, movements: dict, duration: float):
        ...
        """
        start_positions = {leg_num: list(self.current_positions[leg_num]) for leg_num in movements.keys()}
    ...
    def move_forward(self, fast=False):
        ...
        # 다리별로 순차적으로 이동
        if not self._move_legs_interpolated({3: Poses.FORWARD_POS_3}, power_duration): return
        if not self._move_legs_interpolated({1: Poses.FORWARD_POS_5}, power_duration): return
        if not self._move_legs_interpolated({4: Poses.FORWARD_POS_10}, power_duration): return
        if not self._move_legs_interpolated({2: Poses.FORWARD_POS_1}, power_duration): return

        # 모든 다리 원위치
        if not self._move_legs_interpolated({
            1: Poses.FORWARD_POS_0,
            2: Poses.FORWARD_POS_0,
            3: Poses.FORWARD_POS_0,
            4: Poses.FORWARD_POS_0
        }, return_duration): return
    ...
    def turn_left(self):
        ...
        """
        print("--- Turning Left ---")
        power_duration, _ = self._get_durations(fast=False)

        # 각 다리를 순차적으로 이동
        print("Moving Leg 3...")
        if not self._move_legs_interpolated({3: Poses.TURN_LEFT_POS_1}, power_duration): return

        print("Moving Leg 2...")
        if not self._move_legs_interpolated({2: Poses.TURN_LEFT_POS_1}, power_duration): return

        print("Moving Leg 1...")
        if not self._move_legs_interpolated({1: Poses.TURN_LEFT_POS_1}, power_duration): return

        print("Moving Leg 4...")
        if not self._move_legs_interpolated({4: Poses.TURN_LEFT_POS_1}, power_duration): return

        # 모든 다리 동시 이동
        print("Moving all legs together...")
        if not self._move_legs_interpolated({leg: Poses.TURN_LEFT_POS_2 for leg in self.leg_ids}, power_duration): return
        if not self._move_legs_interpolated({leg: Poses.TURN_LEFT_POS_3 for leg in self.leg_ids}, power_duration): return
        if not self._move_legs_interpolated({leg: Poses.TURN_LEFT_POS_4 for leg in self.leg_ids}, power_duration): return
    ...
    def turn_right(self):
        ...
        # 각 다리를 순차적으로 이동
        print("Moving Leg 3...")
        if not self._move_legs_interpolated({3: Poses.TURN_RIGHT_POS_5}, power_duration): return

        print("Moving Leg 2...")
        if not self._move_legs_interpolated({2: Poses.TURN_RIGHT_POS_5}, power_duration): return

        print("Moving Leg 1...")
        if not self._move_legs_interpolated({1: Poses.TURN_RIGHT_POS_5}, power_duration): return

        print("Moving Leg 4...")
        if not self._move_legs_interpolated({4: Poses.TURN_RIGHT_POS_5}, power_duration): return

        # 모든 다리 동시 이동
        print("Moving all legs together...")
        if not self._move_legs_interpolated({leg: Poses.TURN_RIGHT_POS_7 for leg in self.leg_ids}, power_duration): return
        if not self._move_legs_interpolated({leg: Poses.TURN_RIGHT_POS_3 for leg in self.leg_ids}, power_duration): return
        if not self._move_legs_interpolated({leg: Poses.TURN_RIGHT_POS_4 for leg in self.leg_ids}, power_duration): return
