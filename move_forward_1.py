...
START_POS = [2048, 2141, 2228]   # [UPDATE] 's' (Start/Stand)용 (새로운 Pos 0)

# --- [UPDATE] 새로운 보행 시퀀스 (0-4095 원시 위치 값) ---
# 2번, 3번 다리용
GAIT_SEQ_1 = [
...
# --- 보간 설정 ---
STEP_DURATION = 0.25  # 한 보행 스텝에 걸리는 총 시간 (초) (Pos 0->1, 1->2, 2->3 용)
RETURN_STEP_DURATION = STEP_DURATION * 5 # [UPDATE] (Pos 3->4 용)
...
def move_all_legs_sync(target_pos):
    ...
    if dxl_comm_result != COMM_SUCCESS:
        ...
        # [UPDATE] 모든 다리의 현재 위치를 target_pos로 업데이트
        for i in range(1, 5):
...
def move_legs_interpolated_sync(movements, duration=STEP_DURATION):
    """
    [보간 적용] 하나 또는 여러 개의 다리를
    각자의 현재 위치에서 각자의 target_pos까지
    동시에 부드럽게 이동시킵니다. (지정된 duration 동안)
    """
    global current_leg_positions
    ...
    step_time = duration / steps # 파라미터 'duration' 사용
    ...
    # 보간 완료 후, 모든 다리의 최종 위치를 상태에 업데이트
    for move in movements:
...
# --- [UPDATE] 새로운 속도 규칙을 적용한 'move_forward' ---
def move_forward():
    """
    [UPDATE]
    - 순서: 3 -> 2 -> 1 -> 4
    - Power (Pos 0->1, 1->2, 2->3): 정상 속도 (STEP_DURATION)
    - Return (Pos 3->4): 1/5 속도 (RETURN_STEP_DURATION) (비동기 포함)
    """
    print("--- Moving Forward (Cycle: 3->2->1->4) [New Gait/Speed] ---")
    
    # --- 1. 3번 다리 (Power Stroke) [Normal Speed] ---
    print("Moving Leg 3 (Power)... [Normal Speed]")
    if not move_leg_interpolated(LEG3_IDS, 3, GAIT_SEQ_1[1], duration=STEP_DURATION): return
    if not move_leg_interpolated(LEG3_IDS, 3, GAIT_SEQ_1[2], duration=STEP_DURATION): return
    if not move_leg_interpolated(LEG3_IDS, 3, GAIT_SEQ_1[3], duration=STEP_DURATION): return

    # --- 2. 2번 다리 (Power 0->1) + 3번 다리 (Return 3->4) [1/5 Speed] ---
    print("Moving Leg 2 (Power 0->1) + Leg 3 (Return 3->4) [1/5 Speed]...")
    ...
    ], duration=RETURN_STEP_DURATION): return # [UPDATE] 1/5 속도
    
    # Leg 2 나머지 Power Stroke [Normal Speed]
    if not move_leg_interpolated(LEG2_IDS, 2, GAIT_SEQ_1[2], duration=STEP_DURATION): return
    if not move_leg_interpolated(LEG2_IDS, 2, GAIT_SEQ_1[3], duration=STEP_DURATION): return

    # --- 3. 1번 다리 (Power 0->1) + 2번 다리 (Return 3->4) [1/5 Speed] ---
    print("Moving Leg 1 (Power 0->1) + Leg 2 (Return 3->4) [1/5 Speed]...")
    if not move_legs_interpolated_sync([
        {'ids': LEG1_IDS, 'leg_num': 1, 'target_pos': GAIT_SEQ_2[1]}, # Power 0->1
        {'ids': LEG2_IDS, 'leg_num': 2, 'target_pos': GAIT_SEQ_1[4]}  # [ASYNC] Return 3->4
    ], duration=RETURN_STEP_DURATION): return # [UPDATE] 1/5 속도
    
    # Leg 1 나머지 Power Stroke [Normal Speed]
    if not move_leg_interpolated(LEG1_IDS, 1, GAIT_SEQ_2[2], duration=STEP_DURATION): return
    if not move_leg_interpolated(LEG1_IDS, 1, GAIT_SEQ_2[3], duration=STEP_DURATION): return
    
    # --- 4. 4번 다리 (Power 0->1) + 1번 다리 (Return 3->4) [1/5 Speed] ---
    print("Moving Leg 4 (Power 0->1) + Leg 1 (Return 3->4) [1/5 Speed]...")
    if not move_legs_interpolated_sync([
        {'ids': LEG4_IDS, 'leg_num': 4, 'target_pos': GAIT_SEQ_2[1]}, # Power 0->1
        {'ids': LEG1_IDS, 'leg_num': 1, 'target_pos': GAIT_SEQ_2[4]}  # [ASYNC] Return 3->4
    ], duration=RETURN_STEP_DURATION): return # [UPDATE] 1/5 속도
    
    # Leg 4 나머지 Power Stroke [Normal Speed]
    if not move_leg_interpolated(LEG4_IDS, 4, GAIT_SEQ_2[2], duration=STEP_DURATION): return
    if not move_leg_interpolated(LEG4_IDS, 4, GAIT_SEQ_2[3], duration=STEP_DURATION): return

    # --- 5. Cleanup: 4번 다리 (Return 3->4) [1/5 Speed] ---
    print("Finishing cycle: Leg 4 returning [1/5 Speed]...")
    if not move_leg_interpolated(LEG4_IDS, 4, GAIT_SEQ_2[4], duration=RETURN_STEP_DURATION): return # [UPDATE] 1/5 속도
...
def main():
    ...
    print("Press 'w' -> Move Forward (cycle: 3->2->1->4) [New Gait/Speed]")
