...
GAIT_SEQ_1 = [
    ...
    [2561, 1679, 1896], # Pos 2
    [2561, 2141, 2228], # Pos 3
...
def turn_left():
    ...
    if not move_leg_interpolated(LEG3_IDS, 3, GAIT_SEQ_1_REVERSE[3], duration=power_duration): return

    # --- 2. 2번 다리 (REVERSE Power 0->1) + 3번 다리 (REVERSE Return 3->4) ---
    print("Moving Leg 2 (REVERSE Power 0->1) + Leg 3 (REVERSE Return 3->4)...")
    if not move_legs_interpolated_sync([
        {'ids': LEG2_IDS, 'leg_num': 2, 'target_pos': GAIT_SEQ_1_REVERSE[1]}, # REVERSE Power
        {'ids': LEG3_IDS, 'leg_num': 3, 'target_pos': GAIT_SEQ_1_REVERSE[4]}  # [ASYNC] REVERSE Return
