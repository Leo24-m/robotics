...
INTERPOLATION_STEPS = 30 # 보간 단계 수
...
def move_forward(fast=False):
    ...
    if not move_legs_interpolated_sync([
        {'ids': LEG2_IDS, 'leg_num': 2, 'target_pos': GAIT_SEQ_1[6]}, # Power 0->1
        {'ids': LEG4_IDS, 'leg_num': 4, 'target_pos': GAIT_SEQ_1[9]}, # Power 0->1
        {'ids': LEG4_IDS, 'leg_num': 4, 'target_pos': GAIT_SEQ_1[8]}  # [ASYNC] Return 3->4
    ], duration=return_duration): return # [UPDATE] 1/5 속도 (return_duration)
    time.sleep(0.5)



    # --- 1. 3번 다리 (Power Stroke) ---
    print("Moving Leg 3 (Power)...")
    if not move_legs_interpolated_sync([
        {'ids': LEG1_IDS, 'leg_num': 1, 'target_pos': GAIT_SEQ_1[6]}, # Power 0->1
        {'ids': LEG2_IDS, 'leg_num': 2, 'target_pos': GAIT_SEQ_1[9]}, # Power 0->1
        {'ids': LEG4_IDS, 'leg_num': 4, 'target_pos': GAIT_SEQ_1[8]}  # [ASYNC] Return 3->4
    ], duration=return_duration): return # [UPDATE] 1/5 속도 (return_duration)
    time.sleep(0.5)
    if not move_leg_interpolated(LEG3_IDS, 3, GAIT_SEQ_1[1], duration=power_duration): return
    time.sleep(0.5)
    if not move_leg_interpolated(LEG3_IDS, 3, GAIT_SEQ_1[2], duration=power_duration): return
    time.sleep(0.5)
    if not move_leg_interpolated(LEG3_IDS, 3, GAIT_SEQ_1[3], duration=power_duration): return
    
    if not move_legs_interpolated_sync([
        {'ids': LEG1_IDS, 'leg_num': 1, 'target_pos': GAIT_SEQ_1[7]}, # Power 0->1
        {'ids': LEG2_IDS, 'leg_num': 2, 'target_pos': GAIT_SEQ_1[8]}, # Power 0->1
        {'ids': LEG4_IDS, 'leg_num': 4, 'target_pos': GAIT_SEQ_1[9]}  # [ASYNC] Return 3->4
    ], duration=return_duration): return # [UPDATE] 1/5 속도 (return_duration)


    if not move_leg_interpolated(LEG3_IDS, 3, GAIT_SEQ_1[4], duration=power_duration): return
    time.sleep(0.5)
    # time.sleep(1)
    if not move_legs_interpolated_sync([
        {'ids': LEG1_IDS, 'leg_num': 1, 'target_pos': GAIT_SEQ_1[0]}, # Power 0->1
        {'ids': LEG2_IDS, 'leg_num': 2, 'target_pos': GAIT_SEQ_1[0]}, # Power 0->1
        {'ids': LEG4_IDS, 'leg_num': 4, 'target_pos': GAIT_SEQ_1[0]}  # [ASYNC] Return 3->4
    ], duration=return_duration): return # [UPDATE] 1/5 속도 (return_duration)


    # --- 2. 2번 다리 (Power 0->1) + 3번 다리 (Return 3->4) ---
    print("Moving Leg 2 (Power 0->1) + Leg 3 (Return 3->4)...")
    if not move_legs_interpolated_sync([
        {'ids': LEG1_IDS, 'leg_num': 1, 'target_pos': GAIT_SEQ_1[5]}, # Power 0->1
        {'ids': LEG2_IDS, 'leg_num': 2, 'target_pos': GAIT_SEQ_1[1]}, # Power 0->1
        {'ids': LEG3_IDS, 'leg_num': 3, 'target_pos': GAIT_SEQ_1[4]}  # [ASYNC] Return 3->4
    ], duration=return_duration): return # [UPDATE] 1/5 속도 (return_duration)
    time.sleep(0.5)
    
    # Leg 2 나머지 Power Stroke
    if not move_leg_interpolated(LEG2_IDS, 2, GAIT_SEQ_1[2], duration=power_duration): return
    time.sleep(0.5)
    if not move_leg_interpolated(LEG2_IDS, 2, GAIT_SEQ_1[3], duration=power_duration): return
    time.sleep(0.5)

    # --- 3. 1번 다리 (Power 0->1) + 2번 다리 (Return 3->4) ---
    print("Moving Leg 1 (Power 0->1) + Leg 2 (Return 3->4)...")
    if not move_legs_interpolated_sync([
        {'ids': LEG1_IDS, 'leg_num': 1, 'target_pos': GAIT_SEQ_2[1]}, # Power 0->1
        {'ids': LEG2_IDS, 'leg_num': 2, 'target_pos': GAIT_SEQ_1[4]}  # [ASYNC] Return 3->4
    ], duration=return_duration): return # [UPDATE] 1/5 속도 (return_duration)
    time.sleep(0.5)
    ...
    # --- 4. 4번 다리 (Power 0->1) + 1번 다리 (Return 3->4) ---
    print("Moving Leg 4 (Power 0->1) + Leg 1 (Return 3->4)...")
    if not move_legs_interpolated_sync([
        {'ids': LEG4_IDS, 'leg_num': 4, 'target_pos': GAIT_SEQ_2[1]}, # Power 0->1
        {'ids': LEG1_IDS, 'leg_num': 1, 'target_pos': GAIT_SEQ_2[4]}  # [ASYNC] Return 3->4
    ], duration=return_duration): return # [UPDATE] 1/5 속도 (return_duration)
    time.sleep(0.5)
    ...
    # --- 5. Cleanup: 4번 다리 (Return 3->4) ---
    print("Finishing cycle: Leg 4 returning...")
    if not move_leg_interpolated(LEG4_IDS, 4, GAIT_SEQ_2[4], duration=return_duration): return # [UPDATE] 1/5 속도
...
def turn_right():
    ...
    power_duration = STEP_DURATION
    return_duration = RETURN_STEP_DURATION
    print("--- Turning Right (Cycle: 3->2->1->4) ---")
    
    # --- 1. 3번 다리 (NORMAL Power) ---
    print("Moving Leg 3 (NORMAL Power)...")
    if not move_leg_interpolated(LEG3_IDS, 3, GAIT_SEQ_1[1], duration=power_duration): return
    if not move_leg_interpolated(LEG3_IDS, 3, GAIT_SEQ_1[2], duration=power_duration): return
    if not move_leg_interpolated(LEG3_IDS, 3, GAIT_SEQ_1[3], duration=power_duration): return

    # --- 2. 2번 다리 (NORMAL Power 0->1) + 3번 다리 (NORMAL Return 3->4) ---
    print("Moving Leg 2 (NORMAL Power 0->1) + Leg 3 (NORMAL Return 3->4)...")
    if not move_legs_interpolated_sync([
        {'ids': LEG2_IDS, 'leg_num': 2, 'target_pos': GAIT_SEQ_1[1]}, # NORMAL Power
        {'ids': LEG3_IDS, 'leg_num': 3, 'target_pos': GAIT_SEQ_1[4]}  # [ASYNC] NORMAL Return
    ], duration=return_duration): return
    
    # Leg 2 나머지 NORMAL Power
    if not move_leg_interpolated(LEG2_IDS, 2, GAIT_SEQ_1[2], duration=power_duration): return
    if not move_leg_interpolated(LEG2_IDS, 2, GAIT_SEQ_1[3], duration=power_duration): return

    # --- 3. 1번 다리 (REVERSE Power 0->1) + 2번 다리 (NORMAL Return 3->4) ---
    print("Moving Leg 1 (REVERSE Power 0->1) + Leg 2 (NORMAL Return 3->4)...")
    if not move_legs_interpolated_sync([
        {'ids': LEG1_IDS, 'leg_num': 1, 'target_pos': GAIT_SEQ_2_REVERSE[1]}, # REVERSE Power
        {'ids': LEG2_IDS, 'leg_num': 2, 'target_pos': GAIT_SEQ_1[4]}  # [ASYNC] NORMAL Return
    ], duration=return_duration): return
    
    # Leg 1 나머지 REVERSE Power
    if not move_leg_interpolated(LEG1_IDS, 1, GAIT_SEQ_2_REVERSE[2], duration=power_duration): return
    if not move_leg_interpolated(LEG1_IDS, 1, GAIT_SEQ_2_REVERSE[3], duration=power_duration): return
    
    # --- 4. 4번 다리 (REVERSE Power 0->1) + 1번 다리 (REVERSE Return 3->4) ---
    print("Moving Leg 4 (REVERSE Power 0->1) + Leg 1 (REVERSE Return 3->4)...")
    if not move_legs_interpolated_sync([
        {'ids': LEG4_IDS, 'leg_num': 4, 'target_pos': GAIT_SEQ_2_REVERSE[1]}, # REVERSE Power
        {'ids': LEG1_IDS, 'leg_num': 1, 'target_pos': GAIT_SEQ_2_REVERSE[4]}  # [ASYNC] REVERSE Return
    ], duration=return_duration): return
    
    # Leg 4 나머지 REVERSE Power
    if not move_leg_interpolated(LEG4_IDS, 4, GAIT_SEQ_2_REVERSE[2], duration=power_duration): return
    if not move_leg_interpolated(LEG4_IDS, 4, GAIT_SEQ_2_REVERSE[3], duration=power_duration): return

    # --- 5. Cleanup: 4번 다리 (REVERSE Return 3->4) ---
    print("Finishing cycle: Leg 4 returning...")
    if not move_leg_interpolated(LEG4_IDS, 4, GAIT_SEQ_2_REVERSE[4], duration=return_duration): return
