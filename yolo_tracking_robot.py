...
from eval_1st_modi import DynamixelController, QuadrupedRobot, LEG_IDS, DEVICENAME, BAUDRATE

# --- YOLO 모델 설정 ---
MODEL_PATH = "/home/jetson/workspace/Robotics/yolov8n.pt"  # 학습된 모델 경로
CONFIDENCE_THRESHOLD = 0.5  # 신뢰도 임계값
AVAILABLE_CLASSES = ["AI", "AWEAR", "IMR"]  # 사용 가능한 클래스 목록
SELECTED_TARGET_CLASS = "AWEAR"  # 현재 선택된 타겟 클래스 (None이면 수동 선택 모드)

# --- 로봇 제어 설정 ---
CENTER_THRESHOLD = 80  # 화면 중심 허용 범위 (픽셀)
TARGET_DISTANCE = 0.30  # 목표 거리 (미터) - 30cm
DISTANCE_TOLERANCE = 0.05  # 거리 허용 오차 (±5cm)
MIN_DISTANCE = 0.15  # 최소 안전 거리 (15cm)
MAX_DISTANCE = 4.5  # 최대 감지 거리 (3m)
...
class YOLOObjectTracker:
    ...
    def get_control_command(self, target):
        ...
        :return: 'forward', 'left', 'right', 'stop', 'none'
        """
        if target is None:
            return 'none'
...
class RobotController:
    ...
    def execute_command(self, command):
        """
        명령에 따라 로봇을 제어합니다.
        :param command: 'forward', 'left', 'right', 'stop', 'none'
...
def visualize_tracking(frame, depth_frame, target, all_detections, command):
    """
    객체 추적 결과를 시각화합니다.
    """
...
def main():
    ...
    try:
        while True:
            ...
            if robot_controller.is_moving:
                ...
                if last_detections:
                    ...
                    if color_frame_raw:
                        ...
                        # 이전 결과로 화면만 표시
                        display_frame = visualize_tracking(color_frame, None, last_target, last_detections, last_command)
            ...
            # 시각화
            display_frame = visualize_tracking(color_frame, depth_frame, target, detections, command)
